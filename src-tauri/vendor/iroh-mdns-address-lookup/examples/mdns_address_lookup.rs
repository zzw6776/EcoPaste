...
use iroh::{Endpoint, EndpointId, endpoint::presets, endpoint_info::UserData};
...
async fn main() -> Result<()> {
    ...
    let mdns = MdnsAddressLookup::builder().build(endpoint_id)?;
    ...
    for _ in 0..endpoint_count {
        ...
        set.spawn(async move {
            let ep = Endpoint::bind(presets::N0).await?;
            ep.address_lookup()?
                .add(MdnsAddressLookup::builder().build(ep.id())?);
