...
impl MdnsAddressLookupBuilder {
    ...
    /// Sets the IPv4 interfaces used to send and receive multicast discovery packets.
    ///
    /// An empty set retains the default-interface behavior from `swarm-discovery`.
    pub fn multicast_interfaces_v4(
        mut self,
        interfaces: impl IntoIterator<Item = Ipv4Addr>,
    ) -> Self {
        self.multicast_interfaces_v4 = interfaces.into_iter().collect();
        self
    }
...
impl MdnsAddressLookup {
    ...
    /// Replaces the IPv4 interfaces used for multicast discovery.
    pub async fn set_multicast_interfaces_v4(
        &self,
        interfaces: impl IntoIterator<Item = Ipv4Addr>,
    ) {
        self.sender
            .send(Message::SetMulticastInterfacesV4(
...
mod tests {
    ...
    #[test]
    fn builder_deduplicates_multicast_interfaces() {
        let interface = Ipv4Addr::new(192, 168, 50, 84);
